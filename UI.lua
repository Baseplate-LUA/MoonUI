...
library.newtoggle = function(info)
	...
	for i,v in pairs(sections) do if v.section == info.section then v.yaxis = v.yaxis+20+2 end end
	table.insert(toggles,{button = buttonframe,open = false,callback = info.callback,enabled = info.enabled,ty = y,label = label,borders = borders,section = info.section,tab = info.tab})
	library.updatecursor()
end
--
